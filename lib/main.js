    ...
    b.bundle(function (err, code) {
        ...
        if (config.output && config.output.compress) {
            ...
            if (config.output.type === 'normal') {
                ...
                var route = PATH.dirname(PATH.resolve(path, outputPath)) + '/';
...
module.exports = function (opts) {
    ...
    if (getLength.call(rjsMap) === 0) {
        ...
        if (opts.libraryPath) {
            var libraryMap = getLibraryMap(PATH.resolve(process.cwd(), opts.libraryPath) + '/', {});
        }
