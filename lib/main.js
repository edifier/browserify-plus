 ...
 * return false;
 */
function doBrowserify(content, basePath, charset, config) {
    ...
    b.bundle(function (err, code) {
        if (err) {
            ...
            //browserify编译完成，开始输出
            outputHandle(code, basePath, charset, config,lineation);
        }
    });
}
...
module.exports = function (opts) {
    ...
    if (getLength.call(rjsMap) === 0) {
        ...
        if (opts.libraryPath) {
            ...
            var libraryMap = getLibraryMap(PATH.resolve(opts.libraryPath) + lineation, {});
        }
