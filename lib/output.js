...
function outputHandle(code, basePath, charset, config) {
    ...
    var content = iconv.decode(code, charset),config = config.rjs;
    ...
    if (config.output.type === 'normal') {
        mkdir.sync(path);
        fs.writeFileSync(path + fileName, printOut);
    } else if (config.output.type === 'deep') {
        var p = PATH.relative(path, basePath).replace(/(\.+[\/\\])*/gi, '');
        var route = PATH.dirname(path + p) + PATH.sep;
        mkdir.sync(route);
