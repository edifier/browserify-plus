    ...
    function handle(file, curr, pre) {
        ...
        if (pre === null) {
            ...
            extname != '' && delay(function (path) {
                cb && cb(path, util.testRJS(file)&& opts.rjs ? 'rjs' : extname);
                log(path, 'built');
            });
        } else if (curr.nlink === 0) {
            //延迟处理批量删除
            extname != '' && delay(function (path) {
                cb && cb(path, util.testRJS(file)&& opts.rjs ? 'rjs' : extname, 'remove');
                log(path, 'removed');
            });
        } else {
            cb && cb([file], util.testRJS(file)&& opts.rjs ? 'rjs' : extname);
